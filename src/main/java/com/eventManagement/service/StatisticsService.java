...
public class StatisticsService {
	...
	public List<Statistics> getDashboardStatistics(Long adminId) {
		...
		logger.info("Starting getDashboardStatistics method. Admin ID: " + adminId);
		...
		String query = "SELECT e.event_id, e.event_title, "
				...
				+ "  FROM event_users eu " + "  JOIN event e ON eu.event_id = e.event_id"
				+ "  WHERE eu.admin_id = :adminId " + "  GROUP BY e.event_id, e.event_title";
		try {
			statisticsList = entityManager.createNativeQuery(query, "StatisticsMapping")
					.setParameter("adminId", adminId).getResultList();

		} catch (Exception ex) {
			logger.error("Exception while getting statList: " + ex.getMessage());
		}
		logger.info("Retrieved dashboard statistics successfully. Admin ID: " + adminId);
	    logger.info("Finishing getDashboardStatistics method.");
